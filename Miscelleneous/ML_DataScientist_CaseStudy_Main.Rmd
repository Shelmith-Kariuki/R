...
<br>
```{r,warning=FALSE,message=FALSE,results='asis',fig.width=12,eval=FALSE}
...
# rm(reg_df)
# Fitting model
forest <- randomForest(hits ~ ., data=train[1:100000,] %>% select(-entry_page),ntree=500)
varImpPlot(forest)
...
### 9. Fit the prediction model using an xgboost model.
...
### 10. Fit the prediction model using a neural network model.
...
nnets_model <- nnet(hits ~ .,
                 data=train%>% select(-entry_page), size=20)
